...
export class ReportsGenerator {
  ...
  private addBillsTable(doc: PDFKit.PDFDocument, bills: Bill[]): void {
    ...
    bills.forEach((bill, i) => {
      ...
      doc
        .fontSize(10)
        .text(
          bill.tenant?.name || `Tenant #${bill.tenant_id}`,
          tableLeft,
          yPos,
        );
      ...
      doc.text(
        bill.is_paid ? '$0.00' : `$${bill.total.toFixed(2)}`,
        tableLeft +
          colWidths[0] +
          colWidths[1] +
          colWidths[2] +
  ...
  private addPaymentsTable(doc: PDFKit.PDFDocument, payments: Payment[]): void {
    ...
    payments.forEach((payment, i) => {
      doc
        .fontSize(10)
        .text(
          payment.tenant?.name || `Tenant #${payment.tenant_id}`,
          tableLeft,
          yPos,
        );
      doc.text(
  ...
  private addTenantHeader(
    ...
    doc
      .fontSize(12)
      .text(
        `Apartment: ${tenant.apartment?.name || `#${tenant.apartment_id}`}`,
      );
    doc.text(
  ...
  private addTenantBillsTable(doc: PDFKit.PDFDocument, bills: Bill[]): void {
    ...
    bills.forEach((bill, i) => {
      ...
      const monthName = new Date(bill.year, bill.month - 1, 1).toLocaleString(
        'default',
        { month: 'short' },
      );
      ...
      doc.text(`$${bill.rent.toFixed(2)}`, tableLeft + colWidths[0], yPos);
      doc.text(
        `$${bill.water_bill.toFixed(2)}`,
        tableLeft + colWidths[0] + colWidths[1],
        yPos,
      );
      doc.text(
        `$${bill.gas_bill.toFixed(2)}`,
        tableLeft + colWidths[0] + colWidths[1] + colWidths[2],
        yPos,
      );
      doc.text(
  ...
  private addTenantPaymentsTable(
    doc: PDFKit.PDFDocument,
    payments: Payment[],
  ): void {
    doc.fontSize(16).text('Payments', { underline: true });
